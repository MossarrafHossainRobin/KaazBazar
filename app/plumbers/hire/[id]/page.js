// File: app/plumbers/hire/[id]/page.js
...
const MOCK_PROFILE_DATA = {
  ...
  experienceYears: 10,
  skills: ["Pipe Repair", "Leak Detection", "Drain Cleaning", "Emergency Plumbing"],
  coverImageUrl: "https://source.unsplash.com/random/800x200?plumber,tools",
};


// --- 1. SHARED INPUT COMPONENT (MOVED OUTSIDE TO FIX FOCUS LOSS) ---
    ...
    return (
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 relative">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow-2xl transition-all duration-500 overflow-hidden flex flex-col">
              ...
              <div className="space-y-3 text-sm text-gray-700 pt-3">
                ...
                {profile.phone && (
                  <a
                    ...
                    className="flex items-center justify-center space-x-3 p-2 rounded-lg text-indigo-700 font-semibold hover:bg-indigo-100 transition-colors"
                  >
            ...
            <div className="p-6 border-t mt-auto">
              ...
              <div className="flex flex-wrap gap-2 text-left">
                {profile.skills.map((skill, index) => (
                  <span
                    key={index}
                    className="inline-flex items-center px-3 py-1 text-xs font-medium bg-purple-100 text-purple-800 rounded-full transition-shadow hover:shadow-md cursor-default"
                  >
                    {skill}
                  </span>
                ))}
            ...
            <div className="flex-1 overflow-hidden">
              {activeTab === "hire" && (
                <div className="p-6 h-full overflow-y-auto">
                  <h2 className="text-2xl font-bold text-gray-900 mb-5">Submit a Detailed Project Request</h2>
              ...
              {activeTab === "about" && (
                <div className="p-6 h-full overflow-y-auto">
                  ...
                  <p className="text-gray-700 mb-6 leading-relaxed">
                    **{profile.name}** is a highly-rated Master Plumber with **{profile.experienceYears} years** of experience specializing in residential and light commercial projects. He is dedicated to safety, efficiency, and delivering reliable solutions. With over **{profile.projectsCompleted} completed jobs**, you can trust his expertise.
                  </p>
...
export default function PlumberProfilePage() {
  ...
  const [messages, setMessages] = useState([
    { sender: "plumber", text: `Hello! Ready to discuss your plumbing project. How can I help?`, time: new Date(Date.now() - 60000).toLocaleTimeString() },
  ]);
  ...
  const handleSendMessage = useCallback(() => {
    ...
    setTimeout(() => {
      setIsTyping(false);
      const plumberReply = {
        text: `Thanks! I'm reviewing your request: "${userMessage.text.substring(0, 30)}..."`,
        sender: "plumber",
        time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
      };
      setMessages((prev) => [...prev, plumberReply]);
    }, 2500);
  }, [newMessage]);
  ...
  const validateHireForm = useCallback(() => {
    ...
    if (isNaN(budgetValue) || budgetValue <= 50) errors.budget = "Budget must be greater than $50.";
  ...
  const handleHireSubmit = useCallback((e) => {
    ...
    // Simulate API delay for submission
    setTimeout(() => {
        setIsSubmitting(false);

        const hireMsg = `HIRE REQUEST SUBMITTED: Category: ${hireRequest.category}, Budget: $${hireRequest.budget}`;
        setMessages((prev) => [
            ...prev,
            { sender: "me", text: `✅ ${hireMsg}`, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) },
        ]);
  ...
  }, [validateHireForm, hireRequest]); // Depend on hireRequest and validateHireForm
